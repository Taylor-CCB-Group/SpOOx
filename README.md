...
`marker_file` specifies the path to the marker file (see below)
...
However if a file is already available then its path can be specified in 
...
```
Note - the configuration SHOULD be edited at this point before proceding to the next step. You need to specify the markers as 'Nuclear' and 'Cytoplasm' by setting the respective column / row to 1 or 0. 
...
(optional) For the clustering step (see below), a metadata file describing all the samples contained in the above merged data file can be created using the script, make_metadata.py
...
An R Script can be run  cluster the cells based on the markers.You need to supply the merged cell data file and the markers panel file - see the readme in RScripts

```
Rscript R_scripts/Rphenoclustering.R \
--panel_file <markers.tsv> \
--input_file  mergecellData.tab\ 
