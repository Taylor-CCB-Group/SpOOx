...
### Quick start #######
You can run the pipeline one step at a time (see below), which may be a good idea when running for the first time or for trouble shooting, however if you already have a file that specifies which markers are to be used for segmentation and clustering, the whole pipeline can be run with
```
